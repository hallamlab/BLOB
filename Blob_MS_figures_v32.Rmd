...
author: "Sachia J. Traving, Colleen. T. E. Kellogg, Tetjana Ross, Ryan McLaughlin, Brandon Kieft, Grace Y. Ho, Angelica Pena, Martin Krzywinski, Marie Robert, Steven J. Hallam"
date: '2020-07-22, last edited 2022-07-29'
output: html_document
---

This is the Rmarkdown documentation for all figures and supplementary figures, unless otherwise described, for the manuscript "Prokaryotic responses to a warm temperature anomaly in northeast subarctic Pacific waters".
...
The OTU table was also transformed to proportional abundance, as the VST performs poorly in a indicator species analysis (A. S. Hahn, 2016 Unearthing the influence of soil organic matter removal on population structure and metabolism of soil microbial communities. PhD thesis, https://dx.doi.org/10.14288/1.0340624) which is applied downstreams.
```{r, echo = FALSE}
...
## Supplementary figure S2 and S3 - Beta-diversity
...
## Supplementary figure S4 - Alpha-diversity
...
## Figure 4 and supplementary figure S7-10 - Taxonomy distributions
...
# Boxplot of 1-10
box.fig3 <- P26.plong %>% filter(Phylum_mod %in% c("Alphaproteobacteria",
                              "Thaumarchaeota",
                              "Gammaproteobacteria",
                              "Deltaproteobacteria",
                              "Marinimicrobia (SAR406 clade)",
                              ...
                              "Planctomycetes",
                              "Chloroflexi"))
pbox.fig3 <- ggplot(box.fig3, aes(x=Phylum_mod, y=Abundance)) + geom_boxplot(outlier.size = 1) + theme_classic() +
  theme(axis.text.x = element_text(angle=45,hjust = 1, color="black",size=10),
        ...
        strip.text.y.left = element_text(size=7,color="black",angle=0,hjust=1),
        panel.border = element_rect(fill=NA, color=NA)) + ylim(0, 0.005) +
  ylab("Abundance (%)") + xlab("Phylum")

# Boxplot of 11-20
box.S5a <- P26.plong %>% filter(Phylum_mod %in% c("Actinobacteria",
                              "Verrucomicrobia",
                              "Cyanobacteria",
                              "Acidobacteria",
                              "Gemmatimonadetes",
...
pbox.S5a <- ggplot(box.S5a, aes(x=Phylum_mod, y=Abundance)) + geom_boxplot(outlier.size = 1) + theme_classic() +
  theme(axis.text.x = element_text(angle=45,hjust = 1, color="black",size=10),
        axis.text.y = element_text(angle=0,color="black",size=10),
        axis.title = element_text(color="black",size = 12),
        strip.background = element_blank(),
        ...
        strip.text.y.left = element_text(size=7,color="black",angle=0,hjust=1),
        panel.border = element_rect(fill=NA, color=NA)) + ylim(0, 0.005) +
  ylab("Abundance (%)") + xlab("Phylum") + ylim(0, 0.15)

# Boxplot of 21-29
box.S5b <- P26.plong %>% filter(Phylum_mod %in% c("Proteobacteria",
                              "Margulisbacteria",
                              "Archaea",
                              "Patescibacteria",
                              "Kiritimatiellaeota",
                              ...
                              "Fibrobacteres",
                              "Firmicutes"))
pbox.S5b <- ggplot(box.S5b, aes(x=Phylum_mod, y=Abundance)) + geom_boxplot(outlier.size = 1) theme_classic() +
  theme(axis.text.x = element_text(angle=45,hjust = 1, color="black",size=10),
        axis.text.y = element_text(angle=0,color="black",size=10),
        axis.title = element_text(color="black",size = 12),
        strip.background = element_blank(),
        ...
        strip.text.y.left = element_text(size=7,color="black",angle=0,hjust=1),
        panel.border = element_rect(fill=NA, color=NA)) + ylim(0, 0.005) +
  ylab("Abundance (%)") + xlab("Phylum") + ylim(0,0.006)
...
Depth profiles (showing average abundance and std deviation) of phyla, broken down seasonally or by Blob year or not for supplementary figure
```{r, echo=FALSE}

#Most abundant phyla

#set levels so things plot correctly
box.fig3$Months <- factor(box.fig3$Months, levels = c("February", "June", "August"))

box.fig3$Depths <- factor(box.fig3$Depths, levels = c("10", "25", "50","100", "125","150", "200", "300", "400", "600", "800" ,"1000", "1250", "1500",  "2000", "3000", "4000", "B-10"))

#group data in order to create phylaxmontn depth profiles
grouped<-box.fig3v2 %>% group_by(Phylum_mod, Months, Depths) %>% summarise(meana=mean(Abundance), sd = sd(Abundance), maxa = max(Abundance), mina=min(Abundance)) %>% drop_na(sd)

#line + ribbon plot by top phyla
seasonalribbon_topphyla_OSP_bw<-ggplot(data=grouped, aes(x = Depths, y = meana*100, group = Months)) +
  facet_grid(.~Phylum_mod, scale = "free") + 
  geom_line(aes(color = Months, linetype = Months)) +
  scale_linetype_manual(values=c("dashed", "twodash","solid")) +
  scale_color_manual(values = c("#030202","#808080", "#cccccc")) +
  geom_ribbon(aes(ymin = meana*100 - sd*100, ymax = meana*100 + sd*100, fill = Months), alpha = 0.3) +
  coord_flip()+ scale_x_discrete(limits = rev(levels(as.factor(grouped$Depths)))) +
  scale_fill_manual(values = c("#030202","#808080", "#cccccc")) + ylab("Abundance (%)") + xlab("Depth (m)") +  theme_classic() +
...
#group data in order to create blob vs pre-blob depth profiles
grouped_blob<-box.fig3v2 %>% group_by(Phylum_mod, BlobYear, Depths) %>% summarise(meana=mean(Abundance), sd = sd(Abundance), maxa = max(Abundance), mina=min(Abundance)) %>% drop_na(sd)

blobribbon_topphyla_OSP_color<-ggplot(data=grouped_blob, aes(x = Depths, y = meana*100, group = BlobYear)) +
  facet_grid(.~Phylum_mod, scale = "free") + 
  geom_line(aes(color = BlobYear), size=0.5) +
  scale_color_manual(values = c("#0000FF","#FF0404")) +
  geom_ribbon(aes(ymin = meana*100 - sd*100, ymax = meana*100 + sd*100, fill = BlobYear), alpha = 0.3) +
  coord_flip()+ scale_x_discrete(limits = rev(levels(as.factor(grouped$Depths)))) +
  scale_fill_manual(values = c("#0000FF","#FF0404")) + ylab("Abundance (%)") + xlab("Depth (m)") + theme_classic() +
  theme(axis.text.x = element_text(angle=0,color="black",size=10),
        axis.text.y = element_text(angle=0,color="black",size=10),
        axis.title = element_text(color="black",size = 12),
        strip.background = element_blank(),
        #strip.text.x = element_blank(),
...
#Next most abundant phyla

# group for month specific depth profiles

grouped_s5a<-box.S5a %>% group_by(Phylum_mod, Months, Depths) %>% summarise(meana=mean(Abundance), sd = sd(Abundance), maxa = max(Abundance), mina=min(Abundance)) %>% drop_na(sd)

grouped_s5a$Months <- factor(grouped_s5a$Months, levels = c("February", "June", "August"))

grouped_s5a$Depths <- factor(grouped_s5a$Depths, levels = c("10", "25", "50","100", "125","150", "200", "300", "400", "600", "800" ,"1000", "1250", "1500",  "2000", "3000", "4000", "B-10"))


#line + ribbon plot by top phyla
seasonalribbon_phyla2_OSP_bw<-ggplot(data=grouped_s5a, aes(x = Depths, y = meana*100, group = Months)) +
  facet_grid(.~Phylum_mod, scale = "free") + 
  geom_line(aes(color = Months, linetype = Months)) +
  scale_linetype_manual(values=c("dashed", "twodash","solid")) +
  scale_color_manual(values = c("#030202","#808080", "#cccccc")) +
...
#ggsave(seasonalribbon_phyla2_OSP_bw, filename = "seasonalribbon_phyla2_OSP_bw.pdf", device = cairo_pdf, width = 14, height = 6, units = "in")

# group for blob vs pre-blob depth profiles

grouped_blob_s5a<-box.S5a %>% group_by(Phylum_mod, BlobYear, Depths) %>% summarise(meana=mean(Abundance), sd = sd(Abundance), maxa = max(Abundance), mina=min(Abundance)) %>% drop_na(sd)

blobribbon_phyla2_OSP_color<-ggplot(data=grouped_blob_s5a, aes(x = Depths, y = meana*100, group = BlobYear)) +
  facet_grid(.~Phylum_mod, scale = "free") + 
  geom_line(aes(color = BlobYear), size=0.5) +
#  scale_linetype_manual(values=c("dashed", "solid")) +
  ...
  geom_ribbon(aes(ymin = meana*100 - sd*100, ymax = meana*100 + sd*100, fill = BlobYear), alpha = 0.3) +
  coord_flip()+ scale_x_discrete(limits = rev(levels(as.factor(grouped$Depths)))) +
  #theme(axis.text.x = element_text(angle = 45, hjust = 1), panel.background = element_rect(fill = "white", colour = "grey50")) +
  scale_fill_manual(values = c("#0000FF","#FF0404")) + ylab("Abundance (%)") + xlab("Depth (m)") + theme_classic() +
  theme(axis.text.x = element_text(angle=45,hjust=1,color="black",size=10),
        axis.text.y = element_text(angle=0,color="black",size=10),
        axis.title = element_text(color="black",size = 12),
...
#ggsave(blobribbon_phyla2_OSP_color, filename = "blob_phyla2_OSP_color.pdf", device = cairo_pdf, width = 14, height = 6, units = "in")

# Now the least abundant taxa

#group data for plotting month-specific depth profiles for each phyla

grouped_s5b<-box.S5b %>% group_by(Phylum_mod, Months, Depths) %>% summarise(meana=mean(Abundance), sd = sd(Abundance), maxa = max(Abundance), mina=min(Abundance)) %>% drop_na(sd)

grouped_s5b$Months <- factor(grouped_s5b$Months, levels = c("February", "June", "August"))

grouped_s5b$Depths <- factor(grouped_s5b$Depths, levels = c("10", "25", "50","100", "125","150", "200", "300", "400", "600", "800" ,"1000", "1250", "1500",  "2000", "3000", "4000", "B-10"))


#line + ribbon plot by top phyla
seasonalribbon_phyla3_OSP_bw<-ggplot(data=grouped_s5b, aes(x = Depths, y = meana*100, group = Months)) +
  facet_grid(.~Phylum_mod, scale = "free") + 
  geom_line(aes(color = Months, linetype = Months)) +
  scale_linetype_manual(values=c("dashed", "twodash","solid")) +
  scale_color_manual(values = c("#030202","#808080", "#cccccc")) +
...
#ggsave(seasonalribbon_phyla3_OSP_bw, filename = "seasonalribbon_phyla3_OSP_bw.pdf", device = cairo_pdf, width = 14, height = 6, units = "in")

# group for blob vs pre-blob depth profiles

grouped_blob_s5b<-box.S5b %>% group_by(Phylum_mod, BlobYear, Depths) %>% summarise(meana=mean(Abundance), sd = sd(Abundance), maxa = max(Abundance), mina=min(Abundance)) %>% drop_na(sd)

blobribbon_phyla3_OSP_color<-ggplot(data=grouped_blob_s5b, aes(x = Depths, y = meana*100, group = BlobYear)) +
  facet_grid(.~Phylum_mod, scale = "free") + 
  geom_line(aes(color = BlobYear), size=0.5) +
#  scale_linetype_manual(values=c("dashed", "solid")) +
  scale_color_manual(values = c("#0000FF","#FF0404")) +
...
#ggsave(blobribbon_phyla3_OSP_color, filename = "blob_phyla3_OSP_color.pdf", device = cairo_pdf, width = 14, height = 6, units = "in")

```

## Supplementary figure S6 - Heatmap of indicator abundances
...
## Figure 5 - Indicator summaries

```{r}
library(plyr)
library(ggplot2)

indicator_ASV_table = read.table("indicator_summary_with_tax_and_stats.tsv", header=T, sep="\t", check.names=FALSE)
colnames(indicator_ASV_table)[1] = "otu"
otu_table = read.table("otu_table_SILVA132_tax_noCM_fixed_taxonomy.txt", header=T, sep="\t", check.names = F)

# Calculate mean relative abundance across all samples
rel_abund = otu_table
rownames(rel_abund) = rel_abund$OTU_ID
rel_abund$taxonomy = NULL
rel_abund$OTU_ID = NULL
rel_abund = sweep(rel_abund, 2, colSums(rel_abund), '/')
otu_mean_abund = as.data.frame(rowMeans(rel_abund))
colnames(otu_mean_abund) = "mean_rel_abund"
otu_mean_abund$otu = rownames(otu_mean_abund)

indicator_ASV_data = join(indicator_ASV_table,otu_mean_abund)
#indicator_ASV_data$Configuration = paste(indicator_ASV_data$blob,indicator_ASV_data$depth,sep="__")
indicator_ASV_data$taxon = paste(indicator_ASV_data$order," (",indicator_ASV_data$genus,")", sep = "")
indicator_ASV_data$taxon = gsub("Unassigned_","",indicator_ASV_data$taxon)
indicator_ASV_data$taxon = gsub("_"," ",indicator_ASV_data$taxon)
indicator_ASV_data$taxon = gsub("\\(Gammaproteobacteria\\)","\\(Unassigned Gammaproteobacteria\\)",indicator_ASV_data$taxon)
indicator_ASV_data$taxon = gsub("\\(Alphaproteobacteria\\)","\\(Unassigned Alphaproteobacteria\\)",indicator_ASV_data$taxon)
indicator_ASV_data$phylum = ifelse(grepl("Proteobacteria",indicator_ASV_data$phylum),as.character(indicator_ASV_data$class),as.character(indicator_ASV_data$phylum))
indicator_ASV_data = indicator_ASV_data[order(indicator_ASV_data$blob),]
indicator_ASV_data$phylum = factor(indicator_ASV_data$phylum,levels=unique(indicator_ASV_data$phylum))
indicator_ASV_data$taxon = factor(indicator_ASV_data$taxon,levels=unique(indicator_ASV_data$taxon))

# Create indicator plot aesthetics
sec_axis_mult = 0.05
indicator_ASV_plot = ggplot(data=indicator_ASV_data, aes(x=taxon, y=stat)) +
  geom_bar(aes(fill=blob),stat="identity", position = position_stack(reverse=TRUE),colour = "black", width=0.5, size = 0.25) + 
  stat_summary(data = indicator_ASV_data, aes(x=taxon, y=connectivity*sec_axis_mult), fun="sum", geom="point") +
  scale_fill_manual(values = c("blue", "red"), labels=c("Non-Blob","Blob")) + 
  ylab(label = "Cumulative Indicator Score (Bars)") +
  xlab(label = "")+
  theme_minimal() + 
  scale_x_discrete(position = "top") +
  facet_grid(cols = vars(blob), rows = vars(phylum), scales = "free_y", space = "free", switch="y") +
  coord_flip() +
  scale_y_continuous(sec.axis = sec_axis(~ ./sec_axis_mult,name = "Cumulative Indicator Connectivity (Points)")) +
  guides(fill=guide_legend(ncol=1, title="")) +
  theme_classic() +
  theme(axis.text.x = element_text(angle=0,color="black",size=7),
        axis.text.y = element_text(angle=0,color="black",size=7),
...
ggsave("indicator_ASV_plot.svg", indicator_ASV_plot, device = "svg",scale = 1, width = 8, height = 7, units = "in", dpi = 600)

```

## Supplementary figure S5  - Hiveplot panels
Network analyses were performed on the same subsets of the sequence data used for the indicator analyses, using CoNet125 version 1.1.1.beta  application installed within Cytoscape126 version 3.7.1. 
The hiveplot panels was visualized using Hive panel explorer (Perez et al., 2021 https://doi.org/10.1093/bioinformatics/btaa683 )

## Figure S11 - total abundance (%) of all OTUs assigned to the genus Synechococcus and Prochlorococcus

```{r}
library(tidyverse)
library(ggplot2)
# Prochlorococcus is rare at P26 and although patterns suggest an increase during the blob total abundances of Prochlorococcus is extremely low.
# Therefore, all OTUs are grouped as Cyanobacteria and plotted for each depth.
# Specific depths
Cyanobacteria <- P26 %>% tax_glom(taxrank = "Phylum") %>% 
 transform_sample_counts(., function(x) {x/sum(x)}) %>% 
 psmelt %>%
  filter(Phylum == "Cyanobacteria")

# Combined panel of Synechococcus and Prochlorococcus
Syn.Pro <- plot_grid(Syn.p, Pro.p, ncol = 1, align = "v", labels = c("Synechococcus", "Prochlorococcus"))
save_plot("Syn_Pro_abundance_panel.eps",Syn.Pro, nrow = 2, base_aspect_ratio = 4)

# 10 m
Cyanos10 <- Cyanobacteria %>% filter(Depths == 10)
cyanos10p <- ggplot(Cyanos10, aes(x=Sample, y = Abundance, group = 1)) + geom_line() + geom_point() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
# 25 m
Cyanos25 <- Cyanobacteria %>% filter(Depths == 25) 
cyanos25p <- ggplot(Cyanos25, aes(x=Sample, y = Abundance, group = 1)) + geom_line() + geom_point() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
# 50 m
Cyanos50 <- Cyanobacteria %>% filter(Depths == 50)
cyanos50p <- ggplot(Cyanos50, aes(x=Sample, y = Abundance, group = 1)) + geom_line() + geom_point() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
# 100 m
Cyanos100 <- Cyanobacteria %>% filter(Depths == 100) 
cyanos100p <- ggplot(Cyanos100, aes(x=Sample, y=Abundance, group = 1)) + geom_line() + geom_point() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
# 150 m
Cyanos150 <- Cyanobacteria %>% filter(Depths == 150) 
cyanos150p <- ggplot(Cyanos150, aes(x=Sample, y=Abundance, group = 1)) + geom_line() + geom_point() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
# COMBINED FIGURE
library(gridExtra)
library(cowplot)
Cyano.combined <- plot_grid(cyanos10p,
                            cyanos25p,
                            cyanos50p,
                            cyanos100p, ncol = 1, align = "v", labels = c("A", "B", "C", "D"))
save_plot("Cyano_abundance_panel.eps",Cyano.combined, nrow = 4, base_aspect_ratio = 4)
